...
import com.sttarter.init.LibBuildConfig;
 ...
 */
public class STTSQLiteOpenHelperCallbacks {
    private static final String TAG = STTSQLiteOpenHelperCallbacks.class.getSimpleName();

    public void onOpen(final Context context, final SQLiteDatabase db) {
        if (LibBuildConfig.DEBUG) Log.d(TAG, "onOpen");
        // Insert your db open code here.
    }

    public void onPreCreate(final Context context, final SQLiteDatabase db) {
        if (LibBuildConfig.DEBUG) Log.d(TAG, "onPreCreate");
    ...
    public void onPostCreate(final Context context, final SQLiteDatabase db) {
        if (LibBuildConfig.DEBUG) Log.d(TAG, "onPostCreate");
        // Insert your db creation code here. This is called after your tables are created.
    }

    public void onUpgrade(final Context context, final SQLiteDatabase db, final int oldVersion, final int newVersion) {
        if (LibBuildConfig.DEBUG) Log.d(TAG, "Upgrading database from version " + oldVersion + " to " + newVersion);
