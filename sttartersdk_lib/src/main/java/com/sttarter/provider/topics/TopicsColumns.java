package com.sttarter.provider.topics;
...
public class TopicsColumns implements BaseColumns {
    ...
    public static Group completeRow(Cursor cursor)
    {
        Group group = new Group();
        try
        {
            group.setApp_id(cursor.getString(cursor.getColumnIndexOrThrow(_ID)));
            group.setActive(false);
            group.setCreated_at(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_UPDATED_UNIX_TIMESTAMP)));
            //group.setCreated_by(cursor.getColumnIndexOrThrow(TOPIC_UPDATED_UNIX_TIMESTAMP)+"");
            group.setIs_public(cursor.getInt(cursor.getColumnIndexOrThrow(TOPIC_IS_PUBLIC)));
            //group.setId("");
            Gson gson = new Gson();
            group.setMeta(gson.fromJson(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_META)), GroupMeta.class));
            group.setTopic(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_NAME)));
            group.setType(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_TYPE)));
            group.setUpdated_at(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_UPDATED_UNIX_TIMESTAMP)));

            try {
                Type listType = new TypeToken<List<GroupUser>>() {
                }.getType();
                ArrayList<GroupUser> group_users = (ArrayList<GroupUser>) gson.fromJson(cursor.getString(cursor.getColumnIndexOrThrow(TOPIC_GROUP_MEMBERS)), listType);
                group.setGroup_members(group_users);
            }
